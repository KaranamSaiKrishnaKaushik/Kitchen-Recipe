...
public static class MapEndpointsExtension
{
    ...
    public static void MapEndpoints(this WebApplication app)
    {
        ...
        {
            var authenticationUid = httpContext.User.FindFirst(UserId)?.Value;
        ...
        ) =>
        {
            ...
            var command = new AddUserCommand(userDto, authenticationUid);
        ...
        ) =>
        {
            ...
            var command = new UpdateUserCommand(userDto, authenticationUid);
            var result = await mediator.Send(command);
            ...
            [FromServices] AddRecipeCommandHandler handler) =>
        {
            var authenticationUid = httpContext.User.FindFirst(UserId)?.Value;
            if (string.IsNullOrWhiteSpace(authenticationUid)) return Results.Unauthorized();
            command.AuthenticationUid = authenticationUid;
        ...
        ) =>
        {
            ...
            var recipe = await context.Recipe
                ...
                .FirstOrDefaultAsync(r => r.Id == id && r.AuthenticationUid == authenticationUid);
        ...
        app.MapPost("/add-all-recipes", async (
            ...
            [FromServices] AddRecipeCommandHandler handler) =>
        {
            var authenticationUid = httpContext.User.FindFirst("sub")?.Value;
            if (string.IsNullOrWhiteSpace(authenticationUid)) return Results.Unauthorized();
        ...
        app.MapGet("/fetch-all-recipes", [Authorize] async (HttpContext httpContext,
            [FromServices] DataContext context) =>
        {
            ...
            Console.WriteLine($"Authorization Header: {token}");
            Console.WriteLine($"Firebase UID: {authenticationUid}");
        ...
        app.MapPost("/api/products/all-stores",
            async (List<AllStoresProductsDto> input, AddProductsCommandHandler handler) =>
            {
        ...
        app.MapPost("/api/products/searchByNames",
            async (ProductSearchRequest request, GetProductsQueryHandler handler) =>
            {
                var result = await handler.SearchByNames(request.Source, request.Names);
                return Results.Ok(result);
            });

        app.MapGet("/api/products/all-store-products/by-store/{name}",
            async (string name, GetProductsQueryHandler handler) =>
            {
            ...
            DataContext context) =>
        {
            ...
            var cartItem = await context.ShoppingCart
                .FirstOrDefaultAsync(x => x.ProductId == productId && x.AuthenticationUid == authenticationUid);
            ...
            [FromServices] AddCartBulkHandler handler) =>
        {
            var authenticationUid = httpContext.User.FindFirst(UserId)?.Value;
            if (string.IsNullOrWhiteSpace(authenticationUid)) return Results.Unauthorized();
            await handler.HandleAsync(items, authenticationUid);
            return Results.Ok();
        }).RequireAuthorization();
            ...
            [FromServices] GetShoppingCartQueryHandler handler) =>
        {
            ...
            var products = await handler.GetUserCartAsync(authenticationUid);
            return Results.Ok(products);
        }).RequireAuthorization();
